...
let scenes = [
    ...
    Scene(title: "3 / Start the automatic tour", note: "In the main menu, type S. Then choose tour.", commands: ["# S -> tour", "# LLM mission control with real read-only tools", "# Document detective with visible local evidence", "# Story director with a printed plot twist", "# Optional prepared camera journal"], footer: "Small local models generate the answers. Quality and latency need a real-board rehearsal.", seconds: 12),
    Scene(title: "4 / Let the audience steer", note: "Use S to choose one project instead of the automatic tour.", commands: ["# agent: give the assistant a task; inspect its tool plan", "# detective: ask a question about the local brief", "# story: choose a scene, then supply a plot twist", "", "# The bundled exhibit brief is fictional demo data"], footer: "Tool calls are validated. The model cannot execute arbitrary shell commands.", seconds: 12),
    Scene(title: "5 / Prepare the attached camera", note: "Optional camera preparation is inside the same launcher.", commands: ["# U -> 13 -> install: prepare vision dependencies", "# Then return to the main menu and choose C", "# USB camera URI: v4l2:///dev/video0", "# CSI camera may need: csi://0"], footer: "Rehearse model download and TensorRT compilation before class. Camera may be skipped.", seconds: 12),
    Scene(title: "6 / Scene-memory detective", note: "Choose C -> memory.", commands: ["# Capture five frames of the desk", "# Move, add or remove an ordinary object", "# Keep the camera fixed; press Enter to observe again", "# Compare stable objects, counts and positions", "# Local AI explains only the recorded changes"], footer: "Vision exits before loading the LLM. No observed change means no invented change story.", seconds: 12),
    Scene(title: "7 / AI scavenger hunt", note: "Choose C -> hunt.", commands: ["# The LLM plans a quest with 2 or 3 supported targets", "# Arrange ordinary objects in view; press Enter to scan", "# The camera gathers multi-frame evidence", "# Code scores progress over up to three rounds", "# The model does not decide whether you succeeded"], footer: "Suggested props: cup, bottle, book or phone. Detection errors remain possible.", seconds: 12),
    Scene(title: "8 / Workspace-change journal", note: "Choose C -> journal.", commands: ["# Record three observations over time", "# Change the scene between observations", "# Build an ordered event history", "# Ask the local model to summarize the changes"], footer: "The automatic tour uses timed gaps. An unavailable camera is reported and skipped.", seconds: 11),
    Scene(title: "9 / Exit, recover and repeat", note: "The launcher supervises projects one at a time.", commands: ["# Ctrl+C cancels the active project", "# 0 exits the main menu", "# Logs and camera evidence are in runs/demo-*", "# Close other programs if memory is low", "", "bash scripts/run_demo.sh"], footer: "Utilities stay under U. Rehearse the exact student sequence on the Nano before presenting.", seconds: 12),
]
