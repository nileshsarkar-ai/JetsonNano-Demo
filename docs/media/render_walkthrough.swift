...
let scenes = [
    Scene(title: "Local AI on Jetson Nano", note: "Explore 27 named experiments in language, memory and vision.", commands: ["# 9: Persistent Memory Assistant", "# 14: Sampling Playground", "# 22-25: Prompts, Few-Shot Learning, Triage, Summary", "# 26-28: Prompt Injection, Abstention, Context Recall", "# 29-30: Study Partner and Mystery Character Interview"], footer: "Use docs/EXPERIMENTS.md for the full current list.", seconds: 12),
    Scene(title: "Offline AI / Student showcase", note: "Original Nano 4GB. JetPack 4. Ubuntu 18.04. Python 3.6.", commands: ["# Local assistants, stories and conversations", "# Explore prompts, memory and grounded answers", "# Bring the scene to life with camera projects", "# Prepare once, then run local projects from one menu"], footer: "Instructional commands for your Nano terminal.", seconds: 9),
    ...
    Scene(title: "3 / Named experiments", note: "Select an experiment by number and full name.", commands: ["# 15 = Mission Control: Tool-Planning Assistant", "# LLM mission control with real read-only tools", "# Document detective with visible local evidence", "# Story director with a printed plot twist", "# Optional prepared camera journal"], footer: "Select a named experiment and let the class choose the input.", seconds: 12),
    ...
    Scene(title: "6 / Scene-memory detective", note: "Choose 18: Scene Memory Detective.", commands: ["# Capture five frames of the desk", "# Move, add or remove an ordinary object", "# Keep the camera fixed; press Enter to observe again", "# Compare stable objects, counts and positions", "# Local AI explains only the recorded changes"], footer: "Capture the scene, move an object, then compare the evidence.", seconds: 12),
    Scene(title: "7 / AI scavenger hunt", note: "Choose 19: AI Visual Scavenger Hunt.", commands: ["# The LLM plans a quest with 2 or 3 supported targets", "# Arrange ordinary objects in view; press Enter to scan", "# The camera gathers multi-frame evidence", "# Code scores progress over up to three rounds", "# The model does not decide whether you succeeded"], footer: "Suggested props: cup, bottle, book or phone.", seconds: 12),
...
func render(_ scene: Scene, _ index: Int, _ tick: Int) -> CGImage {
    ...
    color(1,1,1).setFill(); NSRect(x:0,y:0,width:width,height:height).fill()
    text("JETSON NANO  /  PYTHON 3.6",48,662,17,color(0.22,0.47,0.09))
    text(scene.title,48,594,36,color(0.10,0.20,0.16))
    text(scene.note,48,551,21,color(0.29,0.38,0.33))
    color(0.95,0.97,0.93).setFill(); NSBezierPath(roundedRect:NSRect(x:40,y:128,width:1200,height:389),xRadius:12,yRadius:12).fill()
    text("TERMINAL  •  COMMANDS TO RUN ON YOUR NANO",64,480,15,color(0.31,0.40,0.34))
    ...
    for (line, command) in scene.commands.enumerated() where line < visible {
        ...
        text(prefix + command,64,424-CGFloat(line)*43,23,comment ? color(0.29,0.38,0.33) : color(0.11,0.23,0.17),mono:true)
    }
    text(scene.footer,48,83,17,color(0.29,0.38,0.33))
    text("INSTRUCTIONAL COMMAND WALKTHROUGH",48,38,13,color(0.33,0.42,0.37))
    text("\(index+1) / \(scenes.count)",1160,38,15,color(0.33,0.42,0.37))
